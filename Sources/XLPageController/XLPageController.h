...
/*
 *  XLPageController 的缓存设置，默认缓存为无限制，当收到 memoryWarning 时，会自动切换到低缓存模式 (XLPageControllerCachePolicyLoXLemory)，并在一段时间后切换到 High .
    收到多次警告后，会停留在到 XLPageControllerCachePolicyLoXLemory 不再增长
 */
typedef NS_ENUM(NSInteger, XLPageControllerCachePolicy) {
    XLPageControllerCachePolicyDisabled   = -1,  // Disable Cache
    XLPageControllerCachePolicyNoLimit    = 0,   // No limit
    XLPageControllerCachePolicyLoXLemory  = 1,   // Low Memory but may block when scroll
    XLPageControllerCachePolicyBalanced   = 3,   // Balanced ↑ and ↓
    XLPageControllerCachePolicyHigh       = 5    // High
};

typedef NS_ENUM(NSUInteger, XLPageControllerPreloadPolicy) {
    XLPageControllerPreloadPolicyNever     = 0, // Never pre-load controller.
    XLPageControllerPreloadPolicyNeighbour = 1, // Pre-load the controller next to the current.
    XLPageControllerPreloadPolicyNear      = 2  // Pre-load 2 controllers near the current.
};

extern NSString *const XLControllerDidAddToSuperViewNotification;
extern NSString *const XLControllerDidFullyDisplayedNotification;
@protocol XLPageControllerDataSource <NSObject>
@optional

/// 自控制器
- (NSInteger)numbersOfChildControllersInPageController:(XLPageController *)pageController;

/// 每一个index对应的控制器
- (__kindof UIViewController *)pageController:(XLPageController *)pageController viewControllerAtIndex:(NSInteger)index;

/// 每一个index对应的标题
- (NSString *)pageController:(XLPageController *)pageController titleAtIndex:(NSInteger)index;
 ...
 *
 *  @param pageController The parent controller (XLPageController)
 ...
 */
- (void)pageController:(XLPageController *)pageController lazyLoadViewController:(__kindof UIViewController *)viewController withInfo:(NSDictionary *)info;
 ...
 *  @param info           A dictionary that includes some infos, such as: `index` / `title`
 */
- (void)pageController:(XLPageController *)pageController willCachedViewController:(__kindof UIViewController *)viewController withInfo:(NSDictionary *)info;
 ...
 *  @param info           A dictionary that includes some infos, such as: `index` / `title`
 */
- (void)pageController:(XLPageController *)pageController willEnterViewController:(__kindof UIViewController *)viewController withInfo:(NSDictionary *)info;
 ...
 *  @param info           A dictionary that includes some infos, such as: `index` / `title`
 */
- (void)pageController:(XLPageController *)pageController didEnterViewController:(__kindof UIViewController *)viewController withInfo:(NSDictionary *)info;
...
/**
 *  标题选中时的背景图片
 *  The title image when selected.
 */
@property (nonatomic, nullable, strong) UIImage *bgImageSelected;

/**
 *  标题非选择时的背景图片
 *  The title's normal image.
 */
@property (nonatomic, nullable, strong) UIImage *bgImageNormal;
 ...
 */
@property (nonatomic, assign) XLMenuViewStyle menuViewStyle;

@property (nonatomic, assign) XLMenuViewLayoutMode menuViewLayoutMode;
 ...
 *  See `XLPageConst.h` for more information.
 */
...
@property (nonatomic, assign) XLPageControllerCachePolicy cachePolicy;

/** 预加载机制，在停止滑动的时候预加载 n 页 */
@property (nonatomic, assign) XLPageControllerPreloadPolicy preloadPolicy;
...
/** 顶部导航栏 */
@property (nonatomic, nullable, weak) XLMenuView *menuView;

/** 内部容器 */
@property (nonatomic, nullable, weak) XLScrollView *scrollView;
...
/**
 Layout all views in XLPageController
